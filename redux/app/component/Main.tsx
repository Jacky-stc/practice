...
import { addTodo, deleTodo, finishTodo } from '../lib/features/countSlice'
...
const Main = () => {
    ...
    const [selectedTodoContent, setSelectedTodoContent] = useState<string>("")
    ...
    const handleClick = ()=>{
        ...
        dispatch(addTodo({id:new Date().toLocaleString(), text:inputText, finish:false}))
        setInputText("")
    }
    ...
    <Modal isOpen={isOpen} onClose={handleModalClose} onOpenChange={onOpenChange} placement='top' closeButton className='dark' motionProps={{
      ...
      <ModalContent>       
        ...
        <ModalBody>
          ...
          <Textarea value={selectedTodoContent} onChange={(e)=>setSelectedTodoContent(e.target.value)}>
          </Textarea>
        </ModalBody>
        <ModalFooter>
          {selectedTodo?.id}
